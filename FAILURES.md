...
### Cloud release review found unbounded browser and MCP listing growth

- What: Independent release review blocked deployment because browser autosaves retained unlimited full snapshots and MCP listing downloaded every full R2 project twice.
- Where: `src/workspaceApi.ts`, `mcp/cloud-workspace-store.ts`, and `mcp/cloud-server.ts` in the isolated cloud branch.
- When/how: Final review after syncing the updated main branch into the existing Sites implementation.
- Why: The first cloud persistence pass did not impose browser-history limits, and removing a non-atomic summary index had restored full-object listing amplification.
- Impact: Long-lived browser editing could exhaust storage, while enough MCP projects could exceed Worker memory or subrequest limits.
- Tried/result: Replaced primary browser storage with bounded IndexedDB plus a bounded fallback; attached compact summaries atomically as R2 object metadata; added finite quotas and removed the second project read. Focused tests passed.
- Solution: Bound snapshot retention and list from metadata written atomically with each project object.

### Browser IndexedDB read and write were not serialized across tabs

- What: A follow-up review found two tabs could read the same revision and later overwrite the single IndexedDB state value.
- Where: `src/workspaceApi.ts` browser mutation lifecycle.
- When/how: Targeted re-review of the storage-bound fix.
- Why: Revision checks occurred on a detached read before a separate write transaction.
- Impact: Concurrent saves could silently lose one tab's update, including changes to unrelated projects.
- Tried/result: Routed every mutation through a named Web Lock, reread state inside the lock, saved before releasing it, and added a concurrent-save conflict regression. Unsupported IndexedDB browsers now fail visibly instead of risking silent overwrite.
- Solution: Serialize the full read-check-write mutation across tabs.

### Standalone typecheck collected a Node-only test dependency

- What: The first final standalone build failed because the browser TypeScript project included `src/workspaceApi.test.ts`, which imports Node-backed MCP fixtures.
- Where: `tsconfig.app.json` test discovery.
- When/how: Final standalone and Sites production build gate after the persistence fixes.
- Why: The app configuration included all of `src`, including test files that are not production browser inputs.
- Impact: Standalone build returned nonzero; the Sites production build itself completed successfully.
- Tried/result: Excluded `src/**/*.test.ts` and `src/**/*.test.tsx` from the production app typecheck; focused tests and the standalone build then passed.
- Solution: Keep tests in Vitest's test compilation path, not the production browser TypeScript project.
